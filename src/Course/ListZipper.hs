...
instance Functor ListZipper where
  ff <$> ListZipper l f r = ListZipper (ff <$> l) (ff f) (ff <$> r)
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus fa (ListZipper l f r) = ListZipper l (fa f) r
...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus newVal (ListZipper l _ r) = ListZipper l newVal r
...
findLeft p lz = go lz
  where go (ListZipper Nil _ _) = IsNotZ
        go (ListZipper (l:.ls) f r) = case p l of
          True  -> IsZ (ListZipper ls l (f:.r))
          False -> go (ListZipper ls l (f:.r))
...
findRight p lz = go lz
  where go (ListZipper _ _ Nil) = IsNotZ
        go (ListZipper l f (r:.rs)) = case p r of
          True  -> IsZ (ListZipper (f:.l) r rs)
          False -> go (ListZipper (f:.l) r rs)

init :: List a -> List a
init lst = go lst Nil
  where go Nil _ = error "empty list"
        go (_:.Nil) accm = reverse accm
        go (a:.as) accm = go as (a:.accm)

listTail :: List a -> a
listTail Nil = error "empty list"
listTail (a:.Nil) = a
listTail (_:.as) = listTail as
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil f Nil) = ListZipper Nil f Nil
moveLeftLoop (ListZipper Nil f r) = ListZipper ((reverse . init) r ++ (f:.Nil)) (listTail r) Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper Nil f Nil) = ListZipper Nil f Nil
moveRightLoop (ListZipper l f Nil) = ListZipper Nil (listTail l) ((reverse . init) l ++ (f:.Nil))
...
moveLeftN n lz = case moveLeft lz of
...
moveRightN n lz = case moveRight lz of
...
moveLeftN' n (ListZipper l f r) | n == 0 = Right (ListZipper l f r)
                                | n > 0 && n > (length l) = Left (length l)
                                | n > 0 && n <= (length l) =
                                  Right . getIsZ . (moveLeftN n) $ (ListZipper l f r)
                                | n < 0 && (abs n) > (length r) = Left (length r)
                                | n < 0 && (abs n) <= (length r) =
                                  Right . getIsZ . (moveRightN (abs n)) $ (ListZipper l f r)
                                | otherwise = error "otherwise. Should not happen."
                                where getIsZ (IsNotZ) = error "getIsZ. Should not happen."
                                      getIsZ (IsZ lz) = lz
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lz = moveLeftN' (n * (-1)) lz
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz = let leftMoveTimes = (index lz) - n
  in case moveLeftN' leftMoveTimes lz of
       Left _ -> IsNotZ
       Right lz' -> IsZ lz'
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  IsNotZ -> error "end. should not happen"
  IsZ lz' -> lz'
...
start lz@(ListZipper l _ _) = case moveLeftN (length l) lz of
  IsNotZ -> error "start. should not happen"
  IsZ lz' -> lz'
