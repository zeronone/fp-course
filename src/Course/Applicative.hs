...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  fs <*> as = flatten $ applyToList <$> fs
    where applyToList f = f <$> as
...
filtering p (x:.xs) = appendIfTrue <$> p x <*> pure x <*> filtering p xs
  where appendIfTrue True x' xs' = (x':.xs')
        appendIfTrue False _ xs' = xs'
