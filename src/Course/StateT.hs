...
runState' ::
  ...
  -> s
  -> (a, s)
runState' state s = case runStateT state s of
  ExactlyOne a -> a
...
instance Monad f => Applicative (OptionalT f) where
  ...
  OptionalT ff <*> OptionalT fa = OptionalT $ do
    ff' <- ff
    case ff' of
      Empty -> pure Empty
      Full ff'' -> do
...
instance Applicative (Logger l) where
  pure a = Logger Nil a
  Logger l1 fa <*> Logger l2 a = Logger (l1 ++ l2) $ fa a
...
instance Monad (Logger l) where
  fa =<< Logger l1 a = case fa a of
    Logger l2 b -> Logger (l1 ++ l2) b
...
type AppT a = StateT (S.Set a) (OptionalT (Logger Chars)) (List a)
...
liftToStateT m = StateT $ \s -> do
  a <- m
  return (a,s)

liftToOptionalT :: Monad m => m a -> OptionalT m a
liftToOptionalT m = OptionalT $ do
  a <- m
  return $ Full a

liftToOptionalTWithEmpty :: Monad m => m a -> OptionalT m a
liftToOptionalTWithEmpty m = OptionalT $ do
  _ <- m
  return Empty
...
distinctG lst = runOptionalT . flip evalT S.empty $ go lst Nil
  where go :: (Integral a, Show a) => List a -> List a -> AppT a
        go Nil accm = liftToStateT . liftToOptionalT . pure . reverse $ accm
        go (a:.as) accm = do
          set <- getT
          if (a > 100)
          then do
            liftToStateT . liftToOptionalTWithEmpty $ log1 ("aborting > 100: " ++ (listh . show) a) Nil
          else do
            let newAccm = if (a `S.member` set) then accm else (a:.accm)
            if (a `mod` 2 == 0)
            then do
              liftToStateT . liftToOptionalT $ log1 ("even number: " ++ (listh . show) a) (a:.accm)
              putT (S.insert a set)
              go as newAccm
            else putT (S.insert a set) *> go as newAccm
